...
const Productmanagement = () => {
  ...
  const {_id,price,category,photo,name,stock} = data?.message || { _id:"",
    photo:"",category : "", name:"", stock:0,price:0
  };
  ...
  const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await updateProduct({formData , userId : user?._id! , productId : data?.message._id!});
  ...
  const deleteHandler = async () => {
    const res = await deleteProduct({userId : user?._id!, productId : data?.message._id!});
